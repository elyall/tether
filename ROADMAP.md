...
| **`tether abandon`** | core (`Repo.abandon`) | Its fate was left open in round 3: it is a convenience over `jj abandon` / `git rebase` plus a `gc` preview. Keep it if the two-clone and remote tests above show it behaves under `git` with remote-tracking branches; otherwise document the VCS commands and remove it. | Removed before 0.1.0, with the `gc --prune-bookmarks` recipe in its place. |
